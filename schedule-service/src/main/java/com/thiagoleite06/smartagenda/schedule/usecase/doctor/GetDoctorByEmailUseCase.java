...
public class GetDoctorByEmailUseCase {

    private final DoctorJpaRepository doctorRepository;

    @Transactional(readOnly = true)
    public Doctor execute(String email) {
        log.info("Getting doctor with email: {}", email);

        DoctorEntity entity = doctorRepository.findByEmail(email)
                .orElseThrow(() -> new IllegalArgumentException("Doctor with email " + email + " not found"));
